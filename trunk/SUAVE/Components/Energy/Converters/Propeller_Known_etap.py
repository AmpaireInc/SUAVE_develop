...
# Propeller_Known_etap.py
#
# Created:  Dec 2023, J. Jacobs
...
# ----------------------------------------------------------------------
## @ingroup Components-Energy-Converters
class Propeller_Known_etap(Energy_Component):
    ...
    def spin(self,conditions):
        ...
        #Unpack current engine parameters
        shp = conditions.propulsion.commanded_power
...
#Start from here?
        ...
        #Calculate Cp and J(advance ratio)
        

        
        V_true = Vv[:,0,None]  #not sure this is the right Velocity
        V_true[V_true < 0] = 1.
        D                       = 2*R                       # prop diameter
        advance_ratio           = V_true / (n * D )           # J = Freestream velocity(m/s) / (Rotational speed(rev/s)*diameter(meters))
        Cp       = shp / (rho_0*(n**3)*(D**5))
        
        #advance_ratio[advance_ratio<.3] = .3
        #advance_ratio[advance_ratio != advance_ratio] = .3
        
        #print("n: ", n[0,0])
     
        #Unpack propeller efficiency table 
        
        eta_vals = self.eta_vals
        Cp_vals = self.Cp_vals
        J_vals = self.J_vals
        
 
        #Lookup values in table and interpolate propeller efficiency (etap)
        
        #Build Interpolator
        prop_table_grid = interp.RegularGridInterpolator((Cp_vals,J_vals), eta_vals,bounds_error=False,fill_value=.15)
        
        #Interpolate value
        
        #print("Cp, J: ", Cp, ", ", advance_ratio)   
        etap = prop_table_grid((Cp,advance_ratio))
        
        #print("Interpolated Eta P: ", etap[0])
        ...
        thrust                  = thp / V_true
        torque                  = shp / omega
        rotor_drag              = (shp - thp) / V_true #crude approximation
        power                   = shp
